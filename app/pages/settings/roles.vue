...
import { useRolesApi } from '~/app/composables/api/useRolesApi';
...
<template>
  <div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-heading font-bold text-gray-900">Roles</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <table v-else class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="role in items" :key="role.id" class="hover:bg-gray-50">
            <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ role.name }}</td>
            <td class="px-6 py-4 text-sm text-gray-500 font-mono">{{ role.codename }}</td>
            <td class="px-6 py-4 text-sm text-gray-500">{{ role.hierarchy_level }}</td>
            <td class="px-6 py-4">
              <span v-if="role.is_system_role" class="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-full">System</span>
            </td>
            <td class="px-6 py-4">
