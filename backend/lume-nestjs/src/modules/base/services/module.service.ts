...
export class ModuleService {
   ...
   * Install a module
   */
  async installModule(moduleInfo: any): Promise<any> {
    // Check dependencies
   ...
   * Uninstall a module
   */
  async uninstallModule(name: string): Promise<any> {
   ...
   * Upgrade a module
   */
  async upgradeModule(name: string, newVersion: string, newManifest: any): Promise<any> {
    return this.prisma.installedModule.update({
      where: { name },
      data: {
        version: newVersion,
   ...
   */
  async getAllPermissions(): Promise<Record<string, any[]>> {
   ...
   */
  async syncPermissions(moduleName: string, manifest: any): Promise<void> {
      ...
      const permData =
        typeof perm === 'string'
          ...
          : {
              ...
              category: perm.category || moduleName,
            };

      const existing = await this.prisma.permission.findUnique({ where: { name: permName } });
   ...
   * Sync module menus
   */
  async syncMenus(moduleName: string, manifest: any): Promise<void> {
   ...
   */
  async syncMenuItem(moduleName: string, menu: any, parentId: number | null = null): Promise<number> {
    const menuData = {
    ...
    let menuId: number;
    if (existing) {
      await this.prisma.menu.update({ where: { id: existing.id }, data: menuData });
      menuId = existing.id;
    } else {
      const created = await this.prisma.menu.create({ data: menuData });
      menuId = created.id;
    }
   ...
   */
  async getDependencyTree(name: string): Promise<any> {
    const mod = await this.prisma.installedModule.findUnique({ where: { name } });
    if (!mod) return null;
