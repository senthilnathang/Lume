...
class GawdesyService extends CRUDMixin {
    ...
    async getSettings() {
        ...
        const settings = await Setting.findAll({
            where: {
                key: { [Symbol.for('like')]: 'gawdesy.%' }
            }
        });

        const result = {};
        for (const s of settings) {
            const key = s.key.replace('gawdesy.', '');
            try {
...
const createGawdesyRouter = (db) => {
    const router = express.Router();

    router.get('/info', async (req, res) => {
        try {
            const service = new GawdesyService(db, req.user ? { user_id: req.user.id } : {});
    ...
    router.get('/modules/status', async (req, res) => {
        try {
            const service = new GawdesyService(db, { user_id: req.user?.id });
    ...
    router.post('/record-rules', async (req, res) => {
        try {
            const service = new GawdesyService(db, { user_id: req.user?.id, company_id: req.user?.company_id });
    ...
    router.get('/hooks', async (req, res) => {
        try {
            const service = new GawdesyService(db);
    ...
    router.get('/export', async (req, res) => {
        try {
            ...
            res.setHeader('Content-Disposition', `attachment; filename=gawdesy-export.${format}`);
            res.json(data);
        } catch (error) {
            res.status(500).json({ success: false, error: error.message });
        }
...
module.exports = { GawdesyService, createGawdesyRouter };
