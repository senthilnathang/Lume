 ...
 * Comprehensive vulnerability scanning and OWASP compliance checking
 * Based on Anthropic Cybersecurity Skills framework
 */

export class SecurityAuditService {
  constructor(prisma, drizzle) {
    this.prisma = prisma;
  ...
  async checkAccessControl() {
    ...
    // Check if role-based access control is properly enforced
  ...
  async checkCryptography() {
    ...
    const hasHttpsEnforcement = settings.some(s =>
      s.key === 'enforce_https' && s.value === 'true'
    );
  ...
  async checkInsecureDesign() {
    ...
    const rateLimitSettings = await this.prisma.setting.findMany({
      where: { key: { contains: 'rate_limit' } }
    });
    ...
    if (rateLimitSettings.length === 0) {
      issues.push({
        ...
        description: 'No rate limiting found to prevent brute force attacks',
  ...
  async checkVulnerableDependencies() {
    ...
    // This should integrate with npm audit or similar
    issues.push({
