...
export default defineEventHandler(async (event) => {
  ...
  const removed = await userService.removeFromGroup(userId, groupId);
  if (!removed) {
    throw createError({ statusCode: 404, statusMessage: 'User not in this group' });
  }

  return successResponse(null, 'User removed from group');
});
