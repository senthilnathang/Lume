...
- Current debt is tracked in `docs/CODE_QUALITY.md` (**124 lint problems / 0 TS errors** as of Phase 3.5 — every rule category except `no-unused-vars` is at 0; the remaining 123 are scattered ≤3/file). Run `npm run lint` from backend/ to see the current count; CI surfaces it in the Action summary.
