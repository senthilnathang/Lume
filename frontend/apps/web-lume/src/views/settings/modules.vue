...
async function handleInstall(mod: Module) {
  Modal.confirm({
    ...
    async onOk() {
      ...
      try {
        const result = await installModule(mod.name);
        if (result.success) {
        ...
        message.error(error?.response?.data?.detail || error.message || 'Failed to install module');
      } finally {
...
async function handleUninstall(mod: Module) {
  Modal.confirm({
    ...
    async onOk() {
      actionLoading.value = true;
      try {
        const result = await uninstallModule(mod.name);
        if (result.success) {
...
async function handleUpgrade(mod: Module) {
  actionLoading.value = true;
  try {
    const result = await upgradeModule(mod.name);
    if (result.success) {
