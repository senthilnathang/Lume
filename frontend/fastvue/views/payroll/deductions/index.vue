...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/payroll/deductions/', { params });
...
function handleCreate() {
  router.push({ name: 'PAYROLLDeductionsCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'PAYROLLDeductionsEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'PAYROLLDeductionsView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/payroll/deductions/${record.id}`);
...
<template>
  <Page
    title="Deductions"
    description="Manage deductions records"
  >
    <Card>
      <!-- Toolbar -->
