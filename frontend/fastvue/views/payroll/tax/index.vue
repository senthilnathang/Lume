...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/payroll/tax/', { params });
...
function handleCreate() {
  router.push({ name: 'PAYROLLTaxCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'PAYROLLTaxEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'PAYROLLTaxView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/payroll/tax/${record.id}`);
