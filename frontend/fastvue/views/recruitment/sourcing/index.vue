...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/recruitment/sourcing/', { params });
...
function handleCreate() {
  router.push({ name: 'RECRUITMENTSourcingCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'RECRUITMENTSourcingEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'RECRUITMENTSourcingView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/recruitment/sourcing/${record.id}`);
...
<template>
  <Page
    title="Sourcing"
    description="Manage sourcing records"
  >
    <Card>
      <!-- Toolbar -->
