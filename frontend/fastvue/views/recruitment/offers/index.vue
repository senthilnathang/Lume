...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/recruitment/offers/', { params });
...
function handleCreate() {
  router.push({ name: 'RECRUITMENTOffersCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'RECRUITMENTOffersEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'RECRUITMENTOffersView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/recruitment/offers/${record.id}`);
...
<template>
  <Page
    title="Offers"
    description="Manage offers records"
  >
    <Card>
      <!-- Toolbar -->
