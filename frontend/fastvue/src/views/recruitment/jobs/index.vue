...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/recruitment/jobs/', { params });
...
function handleCreate() {
  router.push({ name: 'RECRUITMENTJobsCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'RECRUITMENTJobsEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'RECRUITMENTJobsView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/recruitment/jobs/${record.id}`);
...
<template>
  <Page
    title="Jobs"
    description="Manage jobs records"
  >
    <Card>
      <!-- Toolbar -->
