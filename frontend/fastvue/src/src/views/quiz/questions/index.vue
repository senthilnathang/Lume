...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/quiz/questions/', { params });
...
function handleCreate() {
  router.push({ name: 'QUIZQuestionsCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'QUIZQuestionsEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'QUIZQuestionsView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/quiz/questions/${record.id}`);
...
<template>
  <Page
    title="Questions"
    description="Manage questions records"
  >
    <Card>
      <!-- Toolbar -->
