...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/quiz/list/', { params });
...
function handleCreate() {
  router.push({ name: 'QUIZListCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'QUIZListEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'QUIZListView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/quiz/list/${record.id}`);
