...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/payroll/salary/', { params });
...
function handleCreate() {
  router.push({ name: 'PAYROLLSalaryCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'PAYROLLSalaryEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'PAYROLLSalaryView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/payroll/salary/${record.id}`);
