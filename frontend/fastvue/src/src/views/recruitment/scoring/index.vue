...
async function fetchData() {
  ...
  try {
    ...
    const response = await requestClient.get<any>('/recruitment/scoring/', { params });
...
function handleCreate() {
  router.push({ name: 'RECRUITMENTScoringCreate' });
}

function handleEdit(record: any) {
  router.push({ name: 'RECRUITMENTScoringEdit', params: { id: record.id } });
}

function handleView(record: any) {
  router.push({ name: 'RECRUITMENTScoringView', params: { id: record.id } });
}

async function handleDelete(record: any) {
  try {
    await requestClient.delete(`/recruitment/scoring/${record.id}`);
...
<template>
  <Page
    title="Scoring"
    description="Manage scoring records"
  >
    <Card>
      <!-- Toolbar -->
